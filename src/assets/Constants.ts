export const ALERT_MESSAGE = "This user name does not exist! Please specify an existing user name!";
export const ALERT_VISIBLE_TIME = 2000;
export const ERROR_ALERT_MESSAGE = "Something went wrong please try again!";

export const LIST_TITLES = [
    'Name',
    'Rank',
    'No. of bananas',
    'isSearchedUser?'
];
export const LIST_LENGTH = 10;

export const BASE_SCREEN_WIDTH_VALUE = 375;

export const buttonTitles = {
    FUZZY_SEARCH: "Fuzzy Search",
    SEARCH: "Search",
    SHOW_LOWEST_RANKS: "Show lowest ranks",
    SORT_NAMES: "Sort names"
};

export const testID = {
    APP_BUTTON: "app-button",
    APP_LIST: "app-list",
    APP_HOME_SCREEN: "app-home-screen",
    APP_SEARCH_BAR: "app-search-bar",
    APP_TEXT_CONTAINER: "app-text-container",
    APP_TEXT_INPUT: "app-text-input"
}

export const dataUrl = "https://raw.githubusercontent.com/AnkurJDas171/DATA/main/leaderboard.json";
